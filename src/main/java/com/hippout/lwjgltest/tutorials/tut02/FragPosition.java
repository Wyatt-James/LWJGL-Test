...
public class FragPosition extends GLTest {
    ...
    private int initializeProgram()
    {
        ...
        try {
            shaderList.add(Framework.compileShader(ShaderType.VERTEX, shaderLoader.loadText("FragPosition.vert")));
            shaderList.add(Framework.compileShader(ShaderType.FRAGMENT, shaderLoader.loadText("FragPosition.frag")));
        } catch (IOException e) {
            e.printStackTrace();
            System.exit(1);
        }
